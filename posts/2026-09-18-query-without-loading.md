---
title: "셀프 이벤트 수집 및 데이터 파이프라인 구축기 — Athena에 같은 하루를 7가지로 물었더니 답이 3가지였다"
date: 2026-09-18 10:00
summary: dt= 폴더를 나눠놨어도 쿼리가 그 컬럼을 안 쓰면 7일치를 전부 연다. 그리고 파일을 적게 여는 쿼리가 답까지 다르게 낸다.
...
## dt로 거르면 파일 1개, received_at으로 거르면 7개
...
## 50,000 / 50,001 / 49,999 — 셋 다 다른 걸 세고 있었다
...
## 같은 파일 1개에서 SELECT * 는 1.7MB, count(*) 는 19KB
...
## 하루 5,000건으로 줄이면 이 문제가 통째로 사라진다
