---
title: "셀프 이벤트 수집 및 데이터 파이프라인 구축기 — Firehose에서 정하는 네 가지가 읽는 양을 4,800배 가른다"
...
## MinIO를 S3 자리에, DuckDB를 Athena 자리에 놓았다
...
## Firehose 설정 화면에서 정해야 할 게 네 개였다
...
## 답은 7,097건으로 같은데 읽는 양이 92.0MB와 19KB였다
...
## dt= 폴더는 읽을 파일을, Parquet은 읽을 컬럼을 줄인다
...
## Firehose 버퍼를 1분으로 잡으면 하루에 파일 1,440개가 생긴다
...
## duration_ms를 dwell_ms로 바꾸면 평균이 반쪽으로 계산된다
